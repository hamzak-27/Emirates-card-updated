...
def main():
    ...
    if uploaded_file is not None:
        ...
        if hasattr(st.session_state, 'extracted_info') and st.session_state.extracted_info:
            # Create columns for layout only if we have data
            col1, col2 = st.columns(2)
            
            # Display priority fields in col1
            with col1:
                priority_fields = ['name', 'id_number', 'nationality']
                for field in priority_fields:
                    if field in st.session_state.extracted_info:
                        field_label = field.replace('_', ' ').title()
                        st.markdown(f'<div class="field-label">{field_label}</div>', unsafe_allow_html=True)
                        st.markdown(f'<div class="field-value">{st.session_state.extracted_info[field]}</div>', unsafe_allow_html=True)

            # Display any remaining fields in col2
            with col2:
                other_fields = [f for f in st.session_state.extracted_info.keys() if f not in priority_fields]
